...
coal.sources <- SCC[grepl("Coal", SCC$EI.Sector), ]

coal.emissions <- NEI[NEI$SCC %in% coal.sources$SCC,]

total.coal.emissions <- aggregate(Emissions ~ year, data = coal.emissions, sum)

png(filename = "plot4.png", width = 480, height = 480, units = "px")

barplot(total.coal.emissions$Emissions, names.arg = total.coal.emissions$year, xlab = "Year", ylab = "Emissions (Tons of PM2.5)", main = "Total Coal Emissions of PM2.5")
